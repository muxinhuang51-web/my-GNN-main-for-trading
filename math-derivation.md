...
从预测收益最高的簇开始，依次加入簇内股票，直到组合中有效股票数达到目标：

$$\mathcal{S}_t = \bigcup_{j=1}^{J^*} \mathcal{I}_{\text{rank}^{-1}(j)}$$

其中 $J^*$ 为满足以下条件的最小索引：

$$J^* = \min \left\{ J \;\middle|\; \sum_{j=1}^{J} \left|\left\{ i \in \mathcal{I}_{\text{rank}^{-1}(j)} \;\middle|\; r_t^{(i)} \text{ 有效}\right\}\right| \geq \text{target\_portfolio\_valid\_stocks} \right\}$$
